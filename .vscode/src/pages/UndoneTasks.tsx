...
const UndoneTasks = () => {
  const { groupedUndoneTodos, toggleTodo, deleteTodo, editTodo, undoneCount } = useTodos();
            ...
            CATEGORIES.filter((c) => groupedUndoneTodos[c.value]?.length).map((cat, catIdx) => (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2
                    className="text-sm font-body font-bold uppercase tracking-wider"
                    style={{ color: `hsl(var(--${cat.color}))` }}
                  >
                    {cat.label}
                  </h2>
                  <span className="text-xs font-body text-muted-foreground">
                    ({groupedUndoneTodos[cat.value].length})
                  </span>
                  <div className="flex-1 h-px" style={{ background: `hsl(var(--${cat.color}) / 0.2)` }} />
                </div>
                <div
                  className="space-y-3 pl-3"
                  style={{ borderLeft: `3px solid hsl(var(--${cat.color}) / 0.3)` }}
                >
